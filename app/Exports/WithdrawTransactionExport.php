...
class WithdrawTransactionExport implements FromCollection, WithHeadings, WithColumnWidths
{
    protected $withdrawTransactions;
    protected $sumWithdrawAmount;

    // Constructor to inject the data
    public function __construct($withdrawTransactions, $sumWithdrawAmount)
    {
    ...
    {
        $data = collect($this->withdrawTransactions)->map(function ($transaction) {
        ...
        $data->push([
            'कुल निकासी', // Column 1 (will leave it empty for total row)
            '',
            '',
            '',
            $this->sumWithdrawAmount, // The total balance in the last column
